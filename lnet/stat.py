...
class DatasetStat:
    ...
    def compute_requested(self):
        ...
        hist_path = self.path.with_suffix(".hist.npz")
        if hist_path.exists():
            hist_npz: NpzFile = numpy.load(hist_path)
        ...
        cumsums = {name: h.cumsum() for name, h in hist.items()}
        ...
        for name, pers in self.requested.all_percentiles.items():
            ...
            per_values = numpy.searchsorted(cumsums[name], pers * cumsums[name][-1] / 100) * bin_width
    ...
    def get_percentiles(self, name: str, percentiles: Sequence[float]) -> List[float]:
        ...
        if None in ret:
            ...
            return self.get_percentiles(name, percentiles)
        else:
            return ret

    def get_mean_std(self, name: str, percentile_range: Tuple[float, float]) -> Tuple[float, float]:
        mean_std = self.computed.all_mean_std_by_percentile_range[name].get(percentile_range, None)
        if mean_std is None:
            self.request(all_mean_std_by_percentile_range={name: {percentile_range}})
            self.compute_requested()
            return self.get_mean_std(name, percentile_range)
        else:
            return mean_std
