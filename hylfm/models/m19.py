...
class M19(LnetModel):
    def __init__(self, z_out: int, nnum: int, final_activation: Optional[str] = None, n_res2d_1=128):
        super().__init__()
        inplanes = nnum ** 2
        z_valid_cut = 10
        z_out += z_valid_cut
        self.res2d_1 = ResnetBlock(in_n_filters=inplanes, n_filters=n_res2d_1, valid=False)
        # self.res2d_2 = ResnetBlock(in_n_filters=n_res2d_1, n_filters=128, valid=False)
        ...
        self.conv2 = Conv2D(n_res2d_1, z_out * 2, (3, 3), activation="ReLU", initialization=init)
