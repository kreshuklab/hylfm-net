...
class BeadPrecisionRecall(Metric):
    ...
    def __init__(
        ...
        *,
        ...
        super().__init__(name=name, dim_names="zyx" if dim_names is None else dim_names, per_sample=per_sample)

        if not self.per_sample:
            self.update_with_batch = self._update_with_batch

    def _reset_agglomeration(self):
    ...
    def reset(self):
        self._reset_agglomeration()
        self.result_per_sample = []

    def update_with_sample(self, *, prediction, target):
        self._update_with_batch(prediction=prediction[None], target=target[None])
        self.result_per_sample.append(self._compute())
        self._reset_agglomeration()

    def _update_with_batch(self, *, prediction, target):
    ...
    def compute(self) -> Union[List[Dict[str, Any]], Dict[str, Any]]:
        if self.per_sample:
            return list(self.result_per_sample)
        else:
